...
// нашему id 
//post запрос должен менять wallet (скорее всего wallet в этом тестовом должен будет подключить фронтендер через токены)
//проще всего для тестового обойтись обычным fmt.Scan чтобы пользователь вводил свой id wallet = fmt.Scan  
// тогда нужно обязательно построить все вокруг именно переменной wallet 